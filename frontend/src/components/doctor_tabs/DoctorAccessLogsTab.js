...
import "../../css/DoctorAccessLogsTab.css";

const DoctorAccessLogsTab = ({ logs, loading, fetchAccessLogs }) => {
  return (
      ...
      <div className="logs-tab-header">
        <div className="logs-header-group">
          ...
          <div className="logs-header-title">
            ...
            <p>
              View history of all patient data access requests and system activities
            </p>
          </div>
        </div>
        <button
          ...
          disabled={loading}
        >
          {loading ? <FaSpinner className="spin-icon" /> : <FaSync />} Refresh
        </button>
      </div>
      ...
      <div className="logs-table-container">
        {loading ? (
                  ...
                  <tr key={idx}>
                    ...
                    <td>
                      {log.patient_name ? (
                        <div className="log-patient-cell">
                          ...
                          <span className="patient-name-text">{log.patient_name}</span>
                        </div>
                      ) : (
                    ...
                    <td>
                      <div className="log-details">
                        {log.justification || log.details || "No additional details recorded."}
                      </div>
                    </td>
                    <td>
                      <span className={`status-badge ${
                        (log.status === "Granted" || log.status === "Approved" || log.status === "Success") ? "success" : 
                        (log.status === "Denied" || log.status === "Failed") ? "error" : "neutral"
                      }`}>
                        {(log.status === "Granted" || log.status === "Approved" || log.status === "Success") ? <FaCheckCircle className="badge-icon" /> : 
                         (log.status === "Denied" || log.status === "Failed") ? <FaExclamationTriangle className="badge-icon" /> : null}
                        {log.status || "Unknown"}
                      </span>
                    </td>
                  </tr>
          ...
          <div className="logs-empty">
            ...
            <p>Your access history will appear here once you interact with patient records.</p>
          </div>
        )}
      </div>
    </div>
...
export default DoctorAccessLogsTab;
